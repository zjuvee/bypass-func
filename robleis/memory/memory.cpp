...
// created by juve (vestige bypass own)
DWORD get_service_pid(const char* serviceName) {
    DWORD pid = 0;
...
BOOL c_fn_memory::attack_service(const char* serviceName) {
    DWORD process_id = get_service_pid(serviceName);
    if (process_id == 0)
        return false;
